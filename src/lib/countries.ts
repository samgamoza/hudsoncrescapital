// Full ISO 3166-1 country list with dialing codes (curated for clarity).
// Used by admin onboarding, admin client editor, and investor profile.
...
// Strict E.164 regex: leading +, country code 1-3, then 4-14 digits.
const E164 = /^\+[1-9]\d{6,17}$/;

/** Validate an international phone number in E.164 form (e.g. +14155552671). */
export function isValidE164(phone: string): boolean {
  return E164.test(phone.trim());
}

/** Normalise to E.164: strip everything except digits and a leading +. */
export function toE164(input: string, fallbackDial?: string): string {
  ...
  if (!digits) return "";
  return fallbackDial ? `+${fallbackDial}${digits}` : `+${digits}`;
}
